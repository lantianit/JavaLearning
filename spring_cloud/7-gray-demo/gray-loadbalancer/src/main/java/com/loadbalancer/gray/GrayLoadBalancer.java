...
public class GrayLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances,
        ...
        if (instances.isEmpty()) {
            ...
            List<String> headersList = headers.get(GloabVariable.GRAY_TAGE);
            if (headersList != null && headersList.size() > 0 &&
                    headersList.get(0).equals("true")) { // 灰度请求
                // 灰度列表
                List<ServiceInstance> grayList = instances.stream().
                        filter(i -> i.getMetadata().get(GloabVariable.GRAY_TAGE) != null &&
                                i.getMetadata().get(GloabVariable.GRAY_TAGE).equals("true")).
                        toList();
